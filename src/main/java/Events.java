...
public class Events extends Task {
    ...
    private static final DateTimeFormatter TIME_IN  = DateTimeFormatter.ofPattern("HHmm");   // e.g., 1800
    private static final DateTimeFormatter TIME_OUT = DateTimeFormatter.ofPattern("h:mma");  // e.g., 6:00PM
    ...
    public String toString() {
        return String.format(
                "[E]%s (from: %s %s to: %s)",
                super.toString(),
                date.format(DATE_OUT),
                start.format(TIME_OUT),
                end.format(TIME_OUT)
        );
    }
}
